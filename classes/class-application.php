...
class Application {
	...
	public function __construct() {
		$this->plugin_name         = 'combidesk-moneybird';
		$this->plugin_product_name = __( 'Moneybird', $this->plugin_name );
		...
		$this->description = __( 'Synchronize your WooCommerce orders automatically to Moneybird', $this->plugin_name );
		$this->run();
	}
	...
	public function woo_hpos_compatibility() {
		if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', 'combidesk-moneybird/combidesk-moneybird.php', true );
		}
	}
	...
	function add_combidesk_page() {
		...
		$menu_slug  = 'combidesk-moneybird-settings';
	...
	function combidesk_settings() {
		...
		echo '
		  ...
		  <form class="combidesk__cta-form" action="https://combidesk.com/combipackageinstall/woocommerce-moneybird " method="GET" target="_BLANK">
		  ...
		  <form class="combidesk__cta-form" action="https://www.moneybird.nl/" method="GET" target="_BLANK">
			<input type="hidden" name="referrer" value="3eab6305d7" />
			<input type="hidden" name="email_address" value="' . esc_attr( $email_address ) . '" />
	...
	function load_combidesk_plugin_style( $hook ) {
		if ( $hook != 'toplevel_page_combidesk-moneybird-settings' ) {
			return;
		}
